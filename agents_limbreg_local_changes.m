...
video_flag = 1;
if(video_flag)
    vidObj = VideoWriter('dir_mo_without_softening.mp4','MPEG-4');
    open(vidObj);
end
...
for t = 1:Tmax/dt
    ...
    % active_cells = find(pos(:,1)>=1);
    % N_active = length(active_cells);
    % 
    % for i = 1:N_active
    %     cell_index = active_cells(i);
    % 
    %     if pos(cell_index,1) >= 1
    %         cur_kdiv = kdiv_high;
    %     else
    %         cur_kdiv = kdiv;
    %     end
    ...
    % 
    %     r = rand;
    %     if r < (N_active-n_daughter)*cur_kdiv*dt
    ...
    F_pull(active_cells,:) = [pos(active_cells,1)-0.3,zeros(N_active_cells,1)];
    ...
    % Random motion for cells with x < 1
    for i = 1:N_active_cells
        if pos(active_cells(i),1) < 1
            F_pull(active_cells(i),:) = [0,0];
        end
    end
