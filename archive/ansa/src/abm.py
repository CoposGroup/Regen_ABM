"""
Limb Regeneration Simulation - Agents Based Model (2D) - Version 12
Ansa Brew-Smith, December 2025
Copos Lab, Northeastern University

TODO:
    - clean up printing. worth showing length, area, aspect ratio, and ctrl/c59 rmse and/or hausdorff errors
...
def build_boundaries(soft=True):
    ...
    def k_b(x): return KB_MID + (KB_MIN - KB_MID) * expit((x - center) / width)
    if soft:
    ...
    kb_vals = np.where(xe < x_left, KB_MAX, np.where(xe > x_right, kb_vals, KB_MAX * (1 - transition_factor) + kb_vals * transition_factor))
...
@njit(parallel=True, fastmath=True, nogil=True)
def ext_force(Xe, points=np.array(range(96, 104))): ### range(94, 106)
    """
...
def calculate_total_ext_force(Xe, Db, points=np.array(range(96, 104))):
    """
...
def tune_F_per_segment(Xe, Db, desired_force_per_length, points=np.array(range(96, 104))):
    """
...
def single_iteration(step, t):
    ...
    if t >= T_DORMANT:
        cell_cycle(
            div_allowed=True,
            gradient=GRADIENT,
            directed_angle=DIRECTED_DIVISION_ANGLE)
    else:
...
def run_simulation():
    ...
    for step, t in enumerate(np.arange(0, TMAX, DT)):
        ...
        if EXT_STRESS_FORCE and not ext_force_printed and t >= EXT_FORCE_DELAY:
            total_force, total_length, force_per_length = calculate_total_ext_force(
                Xe, Db)
