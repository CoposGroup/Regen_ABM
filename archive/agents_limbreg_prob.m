...
lb = 50;
ly = 100;
sum_blue_data = zeros(lb,num_timesteps);
sum_yellow_data = zeros(ly,num_timesteps);

for i = 1:num_runs
    cell_data = run_simulation();

    for t = 1:num_timesteps
        sum_blue_data(:,t) = sum_blue_data(:,t) + cell_data.blue{t};
        sum_yellow_data(:,t) = sum_yellow_data(:,t) + cell_data.yellow{t};
    end

    all_blue_data = sum_blue_data/num_runs;
    all_yellow_data = sum_yellow_data/num_runs;
...
%     figure(i);
%     histogram(all_blue_data(:,i),'BinWidth',0.1,'Normalization','probability','FaceColor','b');
%     xlim([-2 2])
%     hold on;
% 
%     histogram(all_yellow_data(:,i),'BinWidth',0.1,'Normalization','probability','FaceColor','y');
...
function cell_data = run_simulation()
    ...
    for t = 1:Tmax/dt
        ...
        for i = 1:length(active_cells)
            if norm(F_semi_circle(i,:)) > 0 % indicates collision with boundary
                Fpos = compute_boundary_collision_force(pos(active_cells(i),:),dl_crit,Xb);
        ...
        if mod(t,10000) == 0
            ...
            blue_cells = pos(division_status,:);
            blue_cells = blue_cells(all(~isnan(blue_cells),2),:);
            blue_cells_x = blue_cells(:,1);
    
    
            yellow_cells = pos(~division_status,:);
            yellow_cells = yellow_cells(all(~isnan(yellow_cells),2),:);
            yellow_cells_x = yellow_cells(:,1);
    
    
            % Store data in the structure
            cell_data.blue{end+1} = blue_cells_x;
            cell_data.yellow{end+1} = yellow_cells_x;
    
    
            disp(['Time step: ' num2str(t/10000)]);
    ...
    function [F_semi_circle] = compute_semi_circle_repulsion(pos, dl_crit, Xb)
        ...
        for i = 1:n
            ...
            for j = 1:length(Xb)
                ...
                if r < dl_crit
                    F_semi_circle(i, :) = F_semi_circle(i, :) + [-0.4, 0];
                    %F_semi_circle(i, :)
                    %sprintf('Collision!')
                    %keyboard()
                end
    ...
    function [Fpos] = compute_boundary_collision_force(cell_pos, dl_crit, Xb)
        ...
        % calculate the distances between cell_pos and these points
        distances = sqrt(sum((pts - cell_pos).^2,2));
        
        % find the nearest point
        [~,min_idx] = min(distances);
        nearest_boundary_point = pts(min_idx,:);
