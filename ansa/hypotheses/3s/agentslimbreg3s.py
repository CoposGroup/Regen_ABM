...
SIM 3: RANDOM MOTION AND AP ORIENTED DIVISION ANGLE
    - cells have a random motion term eta
    - division angle is pi/2 with s.d. pi/6
"""
import warnings
...
OUTPUT_DIR = os.path.join('data', f'3s')
...
def cell_cycle(active, t_next):
    ...
    if np.random.rand() < KDIV/(KDIV+KDEATH):
        ...
        if len(free_slots) > 0:
            ...
            if mom is not None:
                dau = free_slots[0]
                ang = np.random.normal(loc=np.pi/2, scale=np.pi/6) ## proximal distal oriented division (angle pi/2 w/ s.d. pi/6)
