...
SIM 6: RANDOM MOTION AND RANDOM DIVISION ANGLE + NONCENTERED SOFT REGION
    - cells have a random motion term eta
    - division angle is ranfom uniform distribution (0, 2 pi)
    - 1 noncentered soft region
"""
import warnings
...
OUTPUT_DIR = os.path.join('data', f'6')
    ...
    for j in prange(n_boundary):
        ...
        if closest_cell >= 0 and min_dist_sq > 0:
            ...
            if spring_extension > 0:
                ...
                # Also pull cell toward boundary (add to existing forces)f
