...
SIM 5: RANDOM MOTION AND RANDOM DIVISION ANGLE WITH SIDE STIFFENING
    - cells have a random motion term eta
    - division angle is ranfom uniform distribution (0, 2 pi)
    - extra stiff for |y| > 1.0
"""
import warnings
...
OUTPUT_DIR = os.path.join('data', f'5s')
os.makedirs(OUTPUT_DIR, exist_ok=True)
STIFF_FACTOR = 1.5 ## stiffening will happen at the 1/soft_factor --> multiply by this instead of divide
# plot setup
...
def build_boundary():
    ...
    stiff_idx = np.where(((yb >= 1.0) & ((yb <= 1.5))) | ((yb >= -1.5) & ((yb <= -1.0))))[0]

    return Xb0, Xb, Nb, Db, blp0, blm0, dsb, soft_idx, stiff_idx

# Initialize boundary
Xb0, Xb, Nb, Db, blp0, blm0, dsb, soft_idx, stiff_idx = build_boundary()
x_cut = Xb0[:, 0].max()
...
### ANIMATION SETUP ###
def animate_frame(step, t, pos, Xb, pos0, soft_idx=soft_idx, stiff_idx=stiff_idx):
    ...
    for i in range(len(Xb)-1):
        if i in soft_idx:
            ...
            color = 'maroon'   # hard boundary segments
        else:
            color = 'red'
...
def single_iteration(step, t):
    ...
    kb_vals[stiff_idx] *= STIFF_FACTOR
