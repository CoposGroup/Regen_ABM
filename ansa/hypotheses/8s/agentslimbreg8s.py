...
SIM 8: RANDOM MOTION, RANDOM DIVISION ANGLE, AND DIVISION ONLY NEAR BOUNDARY
...
OUTPUT_DIR = os.path.join('data', f'8s')
...
def save_files(data_dict, output_file=None):
    ...
    with open(metadata_file, 'w') as f:
        ...
        density_heatmap(SOFT_RANGE, pos=pos, Xb=Xb, x_cut=x_cut, OUTPUT_DIR=OUTPUT_DIR, bin_size=0.1)
...
def cell_cycle(active, t_next):
    ...
    if np.random.rand() < KDIV/(KDIV+KDEATH):
        ...
        if len(free_slots) > 0:
            middle_cells = active[np.where((pos[active,1] >= -0.5) & ## only pick cells near the middle
                                        (pos[active,1] <= 0.5))[0]]
            shuffled_active = np.random.permutation(middle_cells)
            mom = None

            for i in shuffled_active:
                dx = pos[i,0] - Xb[:,0]
                dy = pos[i,1] - Xb[:,1]
                min_dist = np.min(np.sqrt(dx*dx + dy*dy))
                
                if min_dist < (DL_CRIT*2): ## near the middle AND near the boundary
                    mom = i
                    break
...
def run_simulation():
    ...
    cycle_plot(times, cell_count, N0, OUTPUT_DIR)
