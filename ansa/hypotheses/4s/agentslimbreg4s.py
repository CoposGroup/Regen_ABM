...
SIM 4: RANDOM MOTION + (6% OF CELLS DRIFT TOWARDS DISTAL) AND RANDOM DIVISION ANGLE 
    - cells have a random motion term eta
    - division angle is ranfom uniform distribution (0, 2 pi)
    - 6% of cells drift towards distal end
"""
import warnings
...
OUTPUT_DIR = os.path.join('data', f'4s')
...
N_MIGRANTS = int(0.06 * N0) # number of directed migration cells
active0 = np.where(~np.isnan(pos0[:,0]))[0]
MIGRANT_MASK = np.zeros((CELLS_MAX,), dtype=bool)
random_indices = np.random.choice(N0, N_MIGRANTS, replace=False)
for idx in random_indices:
    MIGRANT_MASK[idx] = True
...
def animate_frame(step, t, pos, Xb, pos0, soft_idx=soft_idx):
    ...
    ax.scatter(pos[active & ~daughters & ~MIGRANT_MASK, 0], pos[active & ~daughters & ~MIGRANT_MASK, 1], s=50,
               facecolor=(170/255, 157/255, 241/255))  # Original cells
    ax.scatter(pos[active & ~daughters & MIGRANT_MASK, 0], pos[active & ~daughters & MIGRANT_MASK, 1], s=50,
               color='red') # Migrant Cells
    ax.scatter(pos[active & daughters & ~MIGRANT_MASK, 0], pos[active & daughters & ~MIGRANT_MASK, 1], s=50,
...
# 6. UPDATE POSITIONS WITH DAMPED VELOCITIES - pick some X% to move towards distal end!

    for i in prange(n_cells):
        if MIGRANT_MASK[i]:  # This cell is a migrant
            pos_active[i, 0] += v_active[i, 0]*dt + abs(eta[i, 0])*dt*(0.75) # migrate in x direction (tune speed)
        else:
