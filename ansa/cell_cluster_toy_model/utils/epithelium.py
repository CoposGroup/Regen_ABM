...
###################
# Add horizontal extensions at y = ±1.5
n_points = int(0.25*2/avg_ds)  # Number of points for 0.25 length segment

# Create top horizontal line
x_extend_top = np.linspace(-1.0, xb_semi[0], n_points)[:-1] # change -1.0 back to -0.25 if issues
y_extend_top = np.full_like(x_extend_top, 1.5)

# Create bottom horizontal line
x_extend_bottom = np.linspace(-1.0, xb_semi[-1], n_points)[:-1] # change -1.0 back to -0.25 if issues
y_extend_bottom = np.full_like(x_extend_bottom, -1.5)
...
np.savetxt("epi0.csv", Xb, delimiter=",", comments='')
print('saved epi0.csv')
