...
SIM 7: RANDOM MOTION AND RANDOM DIVISION ANGLE + CELL INTERCALTION
    - cells have a random motion term eta
    - division angle is ranfom uniform distribution (0, 2 pi)
    - intercalation towards center line
"""
import warnings
...
OUTPUT_DIR = os.path.join('data', f'7s')
...
def cell_operations(pos_active, v_active, Xb, dl_crit, kcoll, dt,
    ...
    F_intercalation = np.zeros((n_cells, 2), dtype=float64)
...
# 2. BOUNDARY-CELL SPRING CONNECTIONS
    ...
    # 5. INTERCALATION TOWARDS CENTERLINE
    for i in prange(n_cells):
        dist_center = np.abs(pos[i, 1]) # distance from center line
        if pos[i, 0] > np.min(Xb[:, 0]): # skip cells out of bounds
            if dist_center > 0.05: # don't overcorrect cells already in center 
                if pos_active[i, 1] > 0:
                    force_direction = -1.0  # Pull downward
                else:
                    force_direction = 1.0   # Pull upward
                distance_factor = max(0.1, 1.0 - (pos_active[i, 0] - x_cut) / 1.0)
                force_strength = K_INTERCAL * distance_factor # tune constant
                F_intercalation[i, 1] = force_strength * force_direction
        
    # 5. CALCULATE VELOCITIES FROM FORCES
    for i in prange(n_cells):
        # Calculate the total force on this cell
        total_force_x = F_cc[i, 0] + 5*F_epid[i, 0] + F_correction[i, 0] + F_intercalation[i, 0]
        total_force_y = F_cc[i, 1] + 5*F_epid[i, 1] + F_correction[i, 1] + F_intercalation[i, 1]
