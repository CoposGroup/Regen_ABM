...
SIM 6: RANDOM MOTION AND RANDOM DIVISION ANGLE + NONCENTERED SOFT REGIONS
    - cells have a random motion term eta
    - division angle is ranfom uniform distribution (0, 2 pi)
    - 2 non centered soft regions
"""
import warnings
...
OUTPUT_DIR = os.path.join('data', f'6_2')
...
def build_boundary():
    ...
    soft_idx = np.where((yb >= SOFT_RANGE[0]) & (yb <= SOFT_RANGE[1]) | (yb >= -SOFT_RANGE[1]) & (yb <= -SOFT_RANGE[0]))[0] # symmetrical regions 
