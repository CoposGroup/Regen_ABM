...
SIM 2: RANDOM MOTION AND PD ORIENTED DIVISION ANGLE
    - cells have a random motion term eta
    - division angle is 0 with s.d. pi/6
"""
import warnings
...
OUTPUT_DIR = os.path.join('data', f'2s')
...
def cell_cycle(active, t_next):
    ...
    if np.random.rand() < KDIV/(KDIV+KDEATH):
        ...
        if len(free_slots) > 0:
            ...
            if mom is not None:
                dau = free_slots[0]
                ang = np.random.normal(loc=0, scale=np.pi/6) ## proximal distal oriented division (angle 0 w/ s.d. pi/6)
