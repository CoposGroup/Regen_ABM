...
video_flag = 0;
if(video_flag)
    vidObj = VideoWriter('dir_mo_dir_celldiv(no_softening).mp4','MPEG-4');
    open(vidObj);
end
...
for t = 1:Tmax/dt
    ...
    % a = -20; b = 20; eta = (b-a).*rand(length(active_cells),2) + a; % Brownian motion
    % pos(active_cells,:) = pos(active_cells,:) + v(active_cells,:)*dt + dt*eta*20;
