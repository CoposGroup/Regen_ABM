...
% 
% Random cell movements + start w/ all points on the circle perimeter
%
% CC (Jan 2024)
...
Fpull = zeros(N,2); % Initialize pulling force to zero



% % randomly select particles
% right_movers = randperm(N,5);
% left_movers = setdiff(randperm(N,2), right_movers);



% % particles in certain region
% x_threshold = 0;



% start w/ all points on the circle perimeter
theta = linspace(0, 2*pi, N+1); % divide circle into N parts
theta(end) = []; % last and first points are same
pos = [r_circle * cos(theta)', r_circle * sin(theta)']; % to Cartesian coordinates
...
for t = 1:Tmax/dt
    ...
    % % randomly select particles
    % Fpull(right_movers,1) = 1;
    % Fpull(left_movers,1) = -1;



    % % particles in certain region
    % for i = 1:N
    %     if pos(i,1) < x_threshold
    %         Fpull(i,1) = -1;
    %     else
    %         Fpull(i,1) = 1;
    %     end
    ...
    %     end
    % end
    Fpull(:,1) = 1.*(pos(:,2)>=0) + -1.*(pos(:,2)<0); % vectorize computation
