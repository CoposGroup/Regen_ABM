...
video_flag = 1;
if(video_flag)
    vidObj = VideoWriter('lateral_squeezing_test3_longer.mp4','MPEG-4');
    open(vidObj);
end
...
cells_above_y1 = find(pos(:,2) >= upper_bdry);
num_cells_above_y1 = length(cells_above_y1);

% Randomly select 50% of the cells above y=1
num_moving_cells_down = round(1 * num_cells_above_y1);
moving_indices_down = cells_above_y1(randperm(num_cells_above_y1,num_moving_cells_down));




% Identify cells below y=-1
cells_below_yn1 = find(pos(:,2) <= lower_bdry);
num_cells_below_yn1 = length(cells_below_yn1);

% Randomly select 50% of the cells below y=-1
num_moving_cells_up = round(1 * num_cells_below_yn1);
moving_indices_up = cells_below_yn1(randperm(num_cells_below_yn1,num_moving_cells_up));
...
for t = 1:Tmax/dt
    ...
    if t==1
        ...
        % Plot moving cells
        scatter(pos(moving_indices_down,1), pos(moving_indices_down,2),100,'o','MarkerFaceColor','r','MarkerEdgeColor','b');
        scatter(pos(moving_indices_up,1), pos(moving_indices_up,2),100,'o','MarkerFaceColor','m','MarkerEdgeColor','b');



        plot(xb,yb,'g.-')
        % plot(xb(40:60),yb(40:60),'r','LineWidth',2);
        yline(0)
        yline(upper_bdry,'--')
        yline(lower_bdry,'--')
    ...
    % Set F_pull for these cells
    F_pull(moving_indices_down,:) = [zeros(num_moving_cells_down,1),-pos(moving_indices_down,2)];
    F_pull(moving_indices_up,:) = [zeros(num_moving_cells_up,1),-pos(moving_indices_up,2)];    
    ...
    % F_pull(active_cells,:) = [zeros(N_active_cells,1),zeros(N_active_cells,1)];
    ...
    rest_indices = find(~(yb >= -0.5 & yb <= 0.5));
    F_semi_circle_elasticity = compute_semi_circle_elasticity(Xb, Db, blp0, blm0, dsb, kb, mid_part_indices, rest_indices);
    ...
    for i=1:n
        for j=1:n
            ...
            if (j~=i)&&(dl<dl_crit)
                ...
                F_repulsive(j,:) = -25*(pos(i,:)-pos(j,:))/dl;
                F_repulsive(i,:) = -25*(pos(j,:)-pos(i,:))/dl;                
            end
        end
    end
